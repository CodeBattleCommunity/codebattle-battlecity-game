...
public class NumberedWormHoles extends PointImpl implements WormHole {
    ...
    public Optional<Point> getExit(Direction direction) {
        if (!linkedWormHoles.isEmpty()) {
            if (linkedWormHoles.size() == 1) {
                return Optional.of(linkedWormHoles.get(0));
            }

            int randomIndex = getRandom().nextInt(linkedWormHoles.size());

            return Optional.of(linkedWormHoles.get(randomIndex));
        } else {
